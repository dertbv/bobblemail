...
- [ ] **Email Research Flag System** - Add research flagging for classification investigation 🔍
  - [ ] Add new first column "🔍 Research" to email details table in web interface
  - [ ] Create research flag checkbox for each email row  
  - [ ] Implement `/api/flag-for-research` endpoint for toggling research flags
  - [ ] Use existing `email_flags` table with new flag type `'RESEARCH'`
  - [ ] Add JavaScript function for research flag toggle functionality
  - [ ] Enable ATLAS to query research-flagged emails when asked to investigate
  - [ ] Test workflow: Flag email → Ask ATLAS → Automatic research analysis
